...
public class VoteSystem
{
    ...
    private Team[]? _allowedTeamsToVote;
    ...
    private CancellationTokenSource? _voteTimeoutTimer;
    private CancellationTokenSource? _playerMessageTimer;
    ...
    )
    {
        ...
        _playerMessageTimer = TimerUtility.Repeat(
            1,
            () =>
            {
                SendVoteMessage();
    ...
    {
        TimerUtility.Kill(_voteTimeoutTimer);
        _voteTimeoutTimer = null;
        TimerUtility.Kill(_playerMessageTimer);
    ...
    public void CastVote(IPlayer player, bool vote)
    {
        if (!CanVote(player))
        {
            return;
        ...
        if (IsCaptainVoteOnly())
        {
            if (
                MatchUtility
                    .GetMemberFromLineup(matchData, player.SteamID.ToString(), player.Name)
                    ?.captain == false
            )
            {
                _gameServer.Message(
    ...
    private void SendVoteMessage()
    {
        ...
        {
            if (isCaptainVoteOnly && _captainSystem.IsCaptain(player, player.Controller.Team) == false)
            {
                continue;
            }
            ...
            if (!CanVote(player))
            {
                ...
                {
                    player.SendCenter(_localizer["vote.other_team", action]);
                }
            ...
            if (_votes.ContainsKey(player.SteamID))
            {
                ...
                {
                    player.SendCenter(_localizer["vote.prompt_count", action]);
                }
                continue;
            }
    ...
    private bool IsCaptainVoteOnly()
    {
        ...
        if (_captainOnly && captains[Team.CT] != null && captains[Team.T] != null)
        {
            return true;
        }
    ...
    private Dictionary<Team, IPlayer?> GetCaptains()
    {
    ...
    private bool CanVote(IPlayer player)
    {
        ...
        return _allowedTeamsToVote.Contains(player.Controller.Team);
    }
    ...
    private void PrintToCenter(CCSPlayerController player, string message)
    {
        IPlayer? target = MatchUtility.Core.PlayerManager.GetPlayerFromController(player);
        if (target == null)
        {
            return;
        }
        target.SendCenter(message);
    }
}
