...
public class KnifeSystem
{
    ...
    public void Stay(CCSPlayerController player)
    {
        ...
        // Deliberately not gated on TeamEmptyForfeitSystem here -- the
        // captain who's present gets to lock in stay/switch whenever they
        // want, even while the opposing team is empty. UpdateMapStatus
        // below re-syncs the empty-team check right as the map flips to
        // Live and re-pauses if needed, instead of blocking the decision
        // itself.
    ...
    private void UpdateDecisionCountdown()
    {
        ...
        // Only the deciding captain needs to see this ticking — CCSPlayerController
        // has its own per-player PrintToCenterAlert (unlike GameServer.Message,
        // which only exposes a global broadcast for Alert). It also ticks
        // smoothly every second, unlike Center text (which has a minimum hold
        // time and visibly jumps/lags when refreshed this often).
        MatchManager? match = _matchService.GetCurrentMatch();
        CCSPlayerController? captain = match?.captainSystem?.GetTeamCaptain(_winningTeam.Value);

        captain?.PrintToCenterAlert(_localizer["knife.decision_countdown", FormatRemaining()]);
    }
